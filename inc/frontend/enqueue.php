...
if( ! function_exists('articled_enqueue_style') ) {
  function articled_enqueue_style() {
    ...
    wp_enqueue_style('articled',  get_stylesheet_uri(), array(), ARTICLED_VERSION, 'all');
    wp_enqueue_style('articled-animation-style',  ARTICLED_CSS . 'articled-animation', '', ARTICLED_VERSION, 'all');
    wp_enqueue_style('articled-mobile',  ARTICLED_CSS . 'responsive', '', ARTICLED_VERSION, 'all');

      if( !is_single() ){
        $blog_layout = intval( abu_ekey('articled_blog_post_layout', $articled_options, '4') );
        if( $blog_layout == 3 || $blog_layout == 4 ) {
          wp_enqueue_style('blog-post-classic', get_template_directory_uri() . '/assets/css/blog-post-classic' .$articled_min . '.css',[], ARTICLED_VERSION, 'all');
        }
      }
...
if( ! function_exists('articled_enqueue_sripts') ) {
 function articled_enqueue_sripts() {
    ...
    wp_enqueue_script('articled',     ARTICLED_JS . 'articled' . $articled_min . '.js', [], ARTICLED_VERSION, false);
